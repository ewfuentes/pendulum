...
Pendulum::State Pendulum::simulate(const Pendulum::State &x_0, const Pendulum::Control &u) const {
    const double inertia = config_.mass * config_.length * config_.length;
    const double torque = - config_.length * config_.mass * config_.gravity * std::sin(x_0[States::POSITION]);
